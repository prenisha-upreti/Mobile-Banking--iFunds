...
int Transaction::saveTransactionLog() {
    try {
        ...
        con = driver->connect("tcp://127.0.0.1:3306", "sqluser", "password");
...
int Transaction::initiateTransaction() {
    ...
    try {
        ...
        while (res->next())
        {
            if (recieverUsername == res->getString("name")) {
                ...
                stmt = con->createStatement();
                res = stmt->executeQuery("SELECT * FROM info WHERE id = 3");
                ...
                while (res->next()) {
                    if (transactionAmount <= res->getDouble("balance")) {
                        ...
                        stmt = con->createStatement();                        
                            stmt->executeUpdate("UPDATE info SET balance = balance +" + to_string(transactionAmount)
                                + "WHERE  id =1" + to_string(recieverWallet_id));
