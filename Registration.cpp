...
pass: //label
    ...
    string query = "INSERT INTO info (name,age,email,password,mpin,balance) VALUES ('" + username + "','" + cage + "', '" + email + "', '" + password + "','" + cmpin + "',0)"; // inserts into database
    stmt->execute(query);
}
...
int Registration::confirm_registration()
{
    ...
    try
    {
        ...
        stmt->execute("CREATE TABLE IF NOT EXISTS info( id int auto_increment, name text(255), age int not null, email text(255), password text(255), mpin int not null,balance int, unique KEY (ID),PRIMARY KEY (name(255)) )"); // CREATES TABLE
